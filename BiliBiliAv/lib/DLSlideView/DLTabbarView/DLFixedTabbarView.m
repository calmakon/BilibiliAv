...
- (void)commonInit{
    ...
    bottomLine_ = [[UIView alloc] initWithFrame:CGRectMake(0, self.bounds.size.height-CGFloatFromPixel(1), self.bounds.size.width, CGFloatFromPixel(1))];
    bottomLine_.backgroundColor = [UIColor lightGrayColor];
...
- (void)layoutTabbar{
    ...
    trackView_.frame = CGRectMake(trackX, self.bounds.size.height-kTrackViewHeight, width, kTrackViewHeight);
    bottomLine_.frame = CGRectMake(0, self.bounds.size.height-CGFloatFromPixel(1), self.bounds.size.width, CGFloatFromPixel(1));
}
