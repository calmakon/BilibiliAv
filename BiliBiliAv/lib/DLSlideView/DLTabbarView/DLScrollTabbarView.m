...
- (void)commonInit{
    ...
    backBtn.frame = CGRectMake(10, 32, 20, 18);
    [backBtn addTarget:self action:@selector(popClick) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:backBtn];
    
    scrollView_ = [[UIScrollView alloc] initWithFrame:CGRectMake(40, 0, self.width-40, self.height)];
...
- (void)setTabbarItems:(NSArray *)tabbarItems{
    if (_tabbarItems != tabbarItems) {
        ...
        for (DLScrollTabbarItem *item in tabbarItems) {
            ...
            label.frame = CGRectMake((item.width-label.bounds.size.width)/2.0f, (height+20-label.bounds.size.height)/2.0f, CGRectGetWidth(label.bounds), CGRectGetHeight(label.bounds));
...
- (void)layoutSubviews{
    ...
    scrollView_.frame = CGRectMake(40, 0, self.width-40, self.height);
    [self layoutTabbar];
}
...
- (void)switchingFrom:(NSInteger)fromIndex to:(NSInteger)toIndex percent:(float)percent{
    ...
    trackView_.frame = CGRectMake(x-15, trackView_.frame.origin.y, width+30, CGRectGetHeight(trackView_.bounds));
}
...
- (void)setSelectedIndex:(NSInteger)selectedIndex{
    if (_selectedIndex != selectedIndex) {
        ...
        if (selectedIndex >= 0 && selectedIndex < [self tabbarCount]) {
            ...
            if (selectedIndex > 0) {
                UIView *leftView = [scrollView_ viewWithTag:kViewTagBase+selectedIndex-2];
                rc = CGRectUnion(rc, leftView.frame);
            }
            if (selectedIndex < [self tabbarCount]-2) {
                UIView *rightView = [scrollView_ viewWithTag:kViewTagBase+selectedIndex+2];
            ...
            trackView_.frame = CGRectMake(trackRc.origin.x-15, trackView_.frame.origin.y, trackRc.size.width+30, CGRectGetHeight(trackView_.bounds));
        }
